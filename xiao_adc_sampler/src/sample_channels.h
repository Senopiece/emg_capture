...
static int16_t sampled_channels_buff[CHANNELS];
...
void init_sample_channels(void) {
  for (int i = 0; i < CHANNELS; i++) {
    ...
    adc_channels[i].acquisition_time = ADC_ACQ_TIME_MAX;
...
#ifdef CONFIG_ADC_CONFIGURABLE_INPUTS
    /* Map each channel to an ADC input pin. For nRF, assume channels 0-5 map to
     * AIN0-AIN5 */
